...
SELECT s.orderid, s.salespersonid, s.customerid, p.productid, p.name, p.price, s.quantity, s.quantity * p.price, s.datesale, year(s.datesale) AS year, month(s.datesale) AS month
